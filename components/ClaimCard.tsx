...
const ClaimCard: React.FC<{ claim: Claim }> = ({ claim }) => {
  ...
  let Icon = MessageSquare; // Default for Neutral Signals
  ...
  return (
    <div 
      className={`relative border border-intuition-border bg-[#0a0f1a] transition-all duration-300 overflow-hidden clip-path-slant group ${expanded ? `border-intuition-primary shadow-[0_0_20px_rgba(0,243,255,0.1)]` : 'hover:border-intuition-border/80'}`}
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 min-w-[140px]">
          ...
          <div className="flex flex-col">
             ...
             <span className="text-sm font-bold text-white truncate max-w-[100px]">{claim.subject.label}</span>
          </div>
        </div>

        {/* Predicate Action */}
        <div className="flex-1 flex flex-col items-center justify-center relative px-4">
           <div className={`flex items-center gap-2 text-xs font-black font-display uppercase tracking-widest ${colorClass}`}>
              <Icon size={14} />
              {/* Display actual predicate (e.g. "IS A", "TRUST", "DISTRUST") */}
              {claim.predicate}
           </div>
           <div className="w-full h-px bg-gradient-to-r from-transparent via-slate-700 to-transparent my-1"></div>
           <div className="text-[10px] text-slate-500 font-mono">CONFIDENCE: {claim.confidence}%</div>
        </div>
        ...
        <div className="flex items-center gap-3 justify-end min-w-[140px]">
          <div className="flex flex-col text-right">
             ...
             <Link 
                ...
                className="text-sm font-bold text-white hover:text-intuition-primary transition-colors truncate max-w-[100px]"
             >
                {claim.object.label}
             </Link>
          </div>
          <div className="w-10 h-10 rounded bg-black border border-intuition-primary/30 flex items-center justify-center overflow-hidden shadow-[0_0_10px_rgba(0,243,255,0.1)]">
             {claim.object.image ? (
        ...
        {/* Date & Expand Arrow - Updated Layout */}
      ...
      {expanded && (
        <div className="bg-black/50 border-t border-white/5 p-4 animate-in slide-in-from-top-2">
           {claim.reason && (
               <div className="mb-4 p-3 bg-intuition-card border border-slate-800 rounded relative">
