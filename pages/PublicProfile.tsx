...
import { User, PieChart as PieIcon, Activity, Zap, Shield, TrendingUp, Layers, RefreshCw, Search, ArrowRight, AlertTriangle, Database, Wallet, Loader2, Fingerprint } from 'lucide-react';
...
const PublicProfile: React.FC = () => {
  ...
  const fetchUserData = async (addr: string) => {
    ...
    try {
      ...
      setPortfolioValue(totalAssetsSum.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 }));

      const mergedHistory = chainHistory.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
      setHistory(mergedHistory);
      
      // FOOTPRINT = History Records + Unique Node Interactions
      setSemanticFootprint(mergedHistory.length || finalPositions.length); 
      ...
      const chartPoints = mergedHistory
          ...
          .map(tx => {
              ...
              try {
                  ...
                  val = assetVal > 0 ? assetVal : parseFloat(formatEther(BigInt(tx.shares || '0'))) * 1.5; // proxy if asset missing
              } catch (e) { val = 0.05; }
              currentRunningVol += val;
  ...
  return (
    <div className="min-h-screen bg-[#020308] pt-8 pb-20 px-4 max-w-7xl mx-auto font-mono">
        ...
        <div className="bg-[#050505] p-10 flex flex-col md:flex-row items-center gap-10 clip-path-slant relative overflow-hidden">
          ...
          <div className="flex-1 text-center md:text-left relative z-10">
            ...
            <div className="flex flex-wrap gap-4 justify-center md:justify-start font-mono text-[9px] font-black uppercase">
              <span className="bg-intuition-primary/10 text-intuition-primary px-3 py-1.5 border border-intuition-primary/30 clip-path-slant">LEVEL: {positions.length > 5 ? 'MASTER_TRADER' : 'RECON_LEVEL_1'}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
          ...
          <div className="bg-black border border-slate-900 p-8 clip-path-slant group hover:border-intuition-success/40 transition-all shadow-2xl relative overflow-hidden">
              <div className="text-[9px] font-black font-mono text-slate-600 uppercase mb-4 tracking-[0.3em] flex items-center gap-3">
                  <TrendingUp size={14} className="text-intuition-success" /> Sentiment_Bias
              </div>
              <div className="relative h-4 bg-slate-900 clip-path-slant mt-6 overflow-hidden flex border border-white/5 p-0.5">
                  <div style={{ width: `${sentimentBias.trust}%` }} className="bg-intuition-success h-full shadow-glow-success transition-all duration-1000"></div>
                  <div style={{ width: `${sentimentBias.distrust}%` }} className="bg-intuition-danger h-full shadow-glow-red transition-all duration-1000"></div>
              </div>
              <div className="flex justify-between text-[10px] mt-3 font-mono font-black uppercase tracking-widest">
                  <span className="text-intuition-success">{sentimentBias.trust.toFixed(0)}% Bullish</span>
                  <span className="text-intuition-danger">{sentimentBias.distrust.toFixed(0)}% Bearish</span>
              </div>
          </div>
      </div>
                          ...
                          <tr key={i} className="hover:bg-white/5 transition-all group relative">
                              <td className="px-10 py-8">
                                  <Link to={`/markets/${p.id}`} className="flex items-center gap-6 group-hover:text-intuition-primary transition-colors">
                                      ...
                                      <div>
                                          <div className="font-black text-sm text-white group-hover:text-intuition-primary transition-colors uppercase leading-none mb-1.5 tracking-tight">{p.atom?.label || p.id.slice(0,8)}</div>
                              ...
                              <td className="px-10 py-8 text-right font-black text-sm text-white">{p.shares.toFixed(4)}</td>
                              <td className="px-10 py-8 text-right font-black text-sm text-intuition-success tracking-tight">{p.value.toLocaleString(undefined, { minimumFractionDigits: 2 })} {CURRENCY_SYMBOL}</td>
                              <td className="px-10 py-8 text-right">
                                  <Link to={`/markets/${p.id}`} className="inline-flex px-6 py-2 bg-white/5 border border-white/10 hover:border-intuition-primary hover:text-intuition-primary text-[9px] font-black uppercase clip-path-slant transition-all tracking-widest">Inspect</Link>
                              </td>
                          </tr>
                      )) : (
